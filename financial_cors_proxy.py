...
# -*- coding: utf-8 -*-
"""
Financial Analysis Dashboard V3 — 一體化本機伺服器
  ① 靜態檔案服務（開啟儀表板 HTML）
  ② 智慧 CORS Proxy（轉發至任意 AI API，目標 URL 由 ?url= query param 決定）

使用方式（只需一個指令）：
  python financial_cors_proxy.py            # 預設 Port 8080
  python financial_cors_proxy.py 9000       # 指定 Port

啟動後：
  1. 瀏覽器開啟 http://localhost:8080
  2. 點選 Financial_Analysis_Dashboard_V3.html
  3. AI 面板 → 進階設定 → CORS 代理：選「本機代理 (localhost:8080)」
...
PROXY_PATH = '/proxy'   # 與 Financial_Analysis_Dashboard_V3.html 固定對應


class FinancialServerHandler(BaseHTTPRequestHandler):
    ...
    def _dir_listing(self, local_dir, url_path):
        ...
        rows = ''.join(rows)
        html = f'''<!DOCTYPE html><html><head><meta charset="utf-8"><title>財務報表分析儀表板 V3</title>
<style>body{{font-family:sans-serif;padding:24px;background:#070e1a;color:#ddd}}
h2{{color:#22d3ee}}a{{color:#7ec8e3;text-decoration:none}}a:hover{{color:#22d3ee}}
table{{border-collapse:collapse;width:100%}}td{{padding:7px 14px;border-bottom:1px solid #1e3a5f}}
</style></head><body>
<h2>📊 財務報表分析儀表板 V3 — 目錄</h2>
<table>{rows}</table>
<hr style="border-color:#1e3a5f;margin-top:24px">
<small style="color:#555">Financial CORS Proxy · Port {PORT} · AI 面板開啟「CORS 代理」即可</small>
</body></html>'''
        data = html.encode()
        self.send_response(200)
